...
| Forged or stale node status | Paired peer or network attacker replays, tampers with, or over-shares a status observation | Paired-node status federation | Explicit expiring read-only grant; paired Ed25519 signature and digest verification; monotonic revision and changing watermark; five-minute snapshot lifetime; bounded field allowlist; transactional deletion on revocation | A currently trusted peer may make false sanitized health claims; replicas never establish execution authority | Two-node signature, tamper, stale revision/watermark, expiry, offline, revocation, API privacy, and GET-only tests |
