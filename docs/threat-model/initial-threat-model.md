...
| Unauthorized or malformed change | Peer submits an invalid revision or action to create receiver work | One-way preparation | Validate request, normalized path, source revision identity/ancestry, policy, and stored capabilities before returning preparation; preparation has no filesystem or state writes | Authenticated transport and transfer apply remain later boundaries | Preparation rejection tests |
