...
try {
    ...
    $expectedCapabilities = @("node.health.read", "node.status.read", "project.visibility.read", "task.readiness.read", "assignment.visibility.read", "result-budget-incident.read", "worker-node.inventory.read", "operator.controls.write")
